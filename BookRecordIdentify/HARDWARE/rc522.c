...
//u8 KEY_A[6]= {0x01, 0x01, 0x01, 0x01, 0x01, 0x01};

u8 KEY_B[6]= {0xff,0xff,0xff,0xff,0xff,0xff};

//u8 KEY_B[6]={0x22, 0x22, 0x22, 0x22, 0x22, 0x22};
    ...
    if (status==MI_OK)
    {
        ...
        //printf("card ID:%d",SN);
			  USART2TxStr("Card ID =");
				...
				USART2TxStr(SN_str);
				LCD_ShowString(16*8,16*11,SN_str);
				BEEP_0();
				delay_ms(20);
			  BEEP_1();
			  Flag_find_card = 1;
        status = PcdSelect(SN);
				copy_str(Data_buf,"Hwdaf0703off        T",21);
				copy_str(&Data_buf[12],SN_str,8);	
    }
    if(status == MI_OK)
    {
        status = MI_ERR;
